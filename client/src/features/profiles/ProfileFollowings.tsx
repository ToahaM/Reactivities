        ...
        <Box>
            ...
            {loadingFollowings ? <Typography>Loading...</Typography> : (
                <Box display='flex' marginTop={3} gap={3} >
                    <Typography>
                        {(activeTab !== 3 && profile?.followingCount === 0) ? `${profile?.displayName} is not following anyone`
                            : null}
                        {(activeTab === 3 && profile?.followersCount === 0) ? `No one is following ${profile?.displayName}`
                            : null}
                    </Typography>
